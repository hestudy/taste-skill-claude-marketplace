...
- **Claude Code marketplace install docs** in README: add marketplace, install full/individual plugins, CLI equivalents, update/reload, and optional team `extraKnownMarketplaces` settings. Marked with `CLAUDE-MARKETPLACE-INSTALL` for sync preservation.
- **Scheduled upstream sync** via `.github/workflows/sync-upstream.yml` + `scripts/sync-upstream.sh`: daily merge from `Leonxlnx/taste-skill`, restore `.claude-plugin/` overlay and README install block, open/update a PR. Supports `mode=direct`, optional `SYNC_UPSTREAM_TOKEN`, and soft-fails with a compare link when Actions cannot create PRs.
