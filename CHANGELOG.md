...
- **Scheduled upstream sync** via `.github/workflows/sync-upstream.yml` + `scripts/sync-upstream.sh`: daily merge from `Leonxlnx/taste-skill`, restore `.claude-plugin/` overlay and README install block, open/update a PR.
